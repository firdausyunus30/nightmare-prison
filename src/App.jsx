...
import { getDbRef, isFirebaseConfigured, getStoredFirebaseConfig, initFirebase } from './firebase';
import { set, get, update, push, remove, onValue, onDisconnect } from 'firebase/database';
import { getRoleDistribution, resolveNightPhase, resolveVotingPhase, checkWinner } from './utils/gameLogic';
...
export default function App() {
  ...
  const handleStartGame = async () => {
    ...
    // Distribute roles
    const roles = getRoleDistribution(numPlayers);
    ...
    updates[`rooms/${roomCode}/timerEnd`] = Date.now() + 15000; // 15 seconds reveal timer
  ...
  const triggerNextPhase = async () => {
    ...
    if (currentStatus === 'role_reveal') {
      ...
      updates[`rooms/${roomCode}/timerEnd`] = Date.now() + 35000; // 35 seconds night actions
    } else if (currentStatus === 'playing') {
      if (currentPhase === 'night') {
        // Night -> Day
        const resolution = resolveNightPhase(currentPlayers, roomCode);
        ...
        if (nextWinner) {
          ...
          updates[`rooms/${roomCode}/players`] = resolution.updatedPlayers;
          updates[`rooms/${roomCode}/story`] = resolution.story;
          updates[`rooms/${roomCode}/timerEnd`] = Date.now() + 60000; // 60 seconds discussion
        }
      } else if (currentPhase === 'day') {
        // Day -> Voting
        updates[`rooms/${roomCode}/phase`] = 'voting';
        updates[`rooms/${roomCode}/timerEnd`] = Date.now() + 35000; // 35 seconds voting
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (room.timerEnd && now >= room.timerEnd) {
        if (room.phase === 'day') {
          // If it's the day discussion, don't auto-transition. Set timer to 0 to signal end.
          if (room.timerEnd !== 0) {
            update(getDbRef(`rooms/${roomCode}`), { timerEnd: 0 });
          }
        } else {
          triggerNextPhase();
        }
  ...
  useEffect(() => {
    ...
    if (room.phase === 'night') {
      ...
      const activeVotersWhoMustVote = Object.values(room.players).filter(p => p.alive && !p.vote);
      if (activeVotersWhoMustVote.length === 0) {
        triggerNextPhase();
      }
    } else if (room.phase === 'day') {
      const alivePlayersList = Object.values(room.players).filter(p => p.alive);
  ...
  const handleTransitionPhase = async (forcePhase) => {
    if (!room || room.hostId !== playerId) return;
  ...
  const renderScreen = () => {
    if (!dbConfigured) {
      return <FirebaseConfig onConfigured={handleDbConfigured} />;
    }
    ...
    if (!roomCode || !room) {
      return (
        <LandingPage 
          ...
          onConfigureDb={() => setDbConfigured(false)}
        />
      );
    }
