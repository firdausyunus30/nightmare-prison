...
let playerId = sessionStorage.getItem('satu_malam_player_id');
if (!playerId) {
  playerId = 'p_' + Math.random().toString(36).substring(2, 9);
  sessionStorage.setItem('satu_malam_player_id', playerId);
}

export default function App() {
  const [dbConfigured] = useState(isFirebaseConfigured());
  const [roomCode, setRoomCode] = useState(() => sessionStorage.getItem('satu_malam_room_code') || '');
  const [nickname, setNickname] = useState(() => sessionStorage.getItem('satu_malam_nickname') || '');
  ...
  useEffect(() => {
    ...
    if (newStatus === 'lobby' && lastStatusRef.current !== 'lobby') {
      ...
      if (newPhase === 'night' && lastPhaseRef.current !== 'night') {
        ...
        playVoteSting();
      } else if (newPhase === 'result' && lastPhaseRef.current !== 'result') {
        playDeathSound();
      }
  ...
  const handleCreateRoom = async (name, gameOptions) => {
    ...
    try {
      ...
      setNickname(name);
      setRoomCode(code);
      sessionStorage.setItem('satu_malam_nickname', name);
      sessionStorage.setItem('satu_malam_room_code', code);
    } catch (err) {
      console.error(err);
  ...
  const handleJoinRoom = async (name, code) => {
    ...
    try {
      ...
      if (!snapshot.exists()) {
        throw new Error("Village not found. Double check your code.");
      }
      ...
      if (roomData.status !== 'lobby') {
        if (roomData.players && roomData.players[playerId]) {
          ...
          setRoomCode(code);
          sessionStorage.setItem('satu_malam_nickname', roomData.players[playerId].name);
          sessionStorage.setItem('satu_malam_room_code', code);
          return;
        } else {
          throw new Error("The night has already begun. You cannot join mid-game.");
        }
      }
      ...
      if (Object.keys(roomData.players || {}).length >= maxPlayers && !roomData.players?.[playerId]) {
        throw new Error(`This village is full (${maxPlayers} players maximum).`);
      }
      ...
      if (nameExists) {
        throw new Error("Nickname is already taken in this village.");
      }
  ...
  const handleExitRoom = () => {
    ...
    setRoom(null);
    setRoomCode('');
    sessionStorage.removeItem('satu_malam_room_code');
  };
  ...
  const triggerNextPhase = async () => {
    ...
    if (currentStatus === 'role_reveal') {
      ...
      if (currentPhase === 'night') {
        ...
        // Voting -> Result reveal (or game over)
        ...
        if (nextWinner) {
          ...
          updates[`rooms/${roomCode}/players`] = resolution.updatedPlayers;
          updates[`rooms/${roomCode}/story`] = resolution.announcedText;
          updates[`rooms/${roomCode}/timerEnd`] = Date.now() + (room.gameOptions?.resultSeconds || 10) * 1000;
        }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const now = Date.now();
      // Voting never auto-resolves — host must End the Council.
      // Result / night / day / role_reveal timers do advance.
      if (room.timerEnd && now >= room.timerEnd && room.phase !== 'voting') {
        triggerNextPhase();
      }
    }, 1000);
