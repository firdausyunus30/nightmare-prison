...
import { Users, LogOut, Play, UserCheck, ShieldAlert, Sparkles, SlidersHorizontal } from 'lucide-react';
import { getRoleDistribution, ROLE_INFO } from '../utils/gameLogic';

export default function LobbyScreen({ roomCode, players, gameOptions, currentPlayerId, onStartGame, onLeaveRoom, onToggleLobbyReady }) {
  ...
  const projectedRoles = getRoleDistribution(numPlayers, gameOptions);
  ...
  const minPlayers = gameOptions?.minPlayers || 3;
  const maxPlayers = gameOptions?.maxPlayers || 10;
  const connectedPlayers = playerList.filter(p => !p.disconnected);
  const nonHostPlayers = connectedPlayers.filter(p => !p.isHost);
  const readyNonHostCount = nonHostPlayers.filter(p => p.lobbyReady).length;
  const allReady = connectedPlayers.length >= minPlayers && readyNonHostCount === nonHostPlayers.length;
  ...
  return (
    <div className="flex flex-col justify-between min-h-screen px-4 py-6">
      ...
      <div className="flex items-center justify-between w-full max-w-md mx-auto mb-6">
        <button
          onClick={onLeaveRoom}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-moon-dark text-xs border border-moon-iron/40 hover:text-red-500 hover:border-red-500/30 transition-all active:scale-95"
        >
        ...
        <div className="flex items-center gap-1 text-[10px] tracking-wider text-gray-500 uppercase font-mono">
          <span className="w-2 h-2 rounded-full bg-moon-safe animate-pulse"></span>
          <span>Synced with Village</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-start w-full max-w-md mx-auto space-y-6">
        ...
        <div className="w-full text-center p-6 rounded-2xl glass-panel metal-border box-glow-red relative overflow-hidden">
          ...
          <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] mb-1">Village Code</p>
          <h2 
        ...
        <div className="w-full flex-1 flex flex-col p-5 rounded-2xl glass-panel metal-border">
          <div className="flex items-center justify-between border-b border-moon-iron/40 pb-3 mb-4">
            <h3 className="text-sm font-semibold tracking-wider text-gray-300 uppercase flex items-center gap-2">
              <Users className="w-4 h-4 text-red-500" />
              <span>Villagers Joined ({numPlayers}/{maxPlayers})</span>
            </h3>
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-center justify-between px-3 py-2.5 rounded-lg border transition-all ${
                    isMe 
                      ? 'bg-moon-blood/25 border-red-500/40 shadow-[inset_0_1px_3px_rgba(220,38,38,0.1)]' 
                      : 'bg-moon-dark/60 border-moon-iron/30'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${player.disconnected ? 'bg-gray-600' : 'bg-moon-safe animate-pulse'}`}></span>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {player.lobbyReady && (
                      <span className="text-[9px] font-bold bg-moon-safe/20 text-moon-safe border border-moon-safe/40 px-1.5 py-0.5 rounded uppercase tracking-wide animate-pulse">
                        ✓ Ready
                      </span>
                    )}
                    {player.disconnected && (
        ...
        <div className="w-full p-4 rounded-xl bg-moon-dark/40 border border-moon-iron/20 text-xs">
          ...
          <div className="grid grid-cols-3 gap-2 text-[10px] text-gray-400 font-mono">
            {Object.entries(roleCounts).map(([role, count]) => {
              ...
              return (
                <div key={role} className="flex items-center gap-1 bg-moon-metal/40 px-2 py-1 rounded border border-moon-iron/10">
        ...
        <div className="w-full p-3 rounded-xl bg-moon-dark/40 border border-moon-iron/20 text-[10px] text-gray-500 flex items-start gap-2">
          <SlidersHorizontal className="w-3.5 h-3.5 text-red-500 shrink-0 mt-0.5" />
          <span>Game setup: {minPlayers}-{maxPlayers} players · Night {gameOptions?.nightSeconds || 35}s · Discussion {gameOptions?.daySeconds || 60}s · Vote {gameOptions?.votingSeconds || 35}s</span>
        </div>
          ...
          <button
            ...
            className={`w-full py-3 rounded-lg font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 border uppercase tracking-widest active:scale-95 ${
              isCurrentPlayerReady
                ? 'bg-moon-safe/20 border-moon-safe/50 text-moon-safe hover:bg-moon-safe/10'
                : 'bg-moon-blood hover:bg-moon-danger border-red-500/20 text-white shadow-[0_0_20px_rgba(220,38,38,0.2)] hover:shadow-[0_0_25px_rgba(220,38,38,0.35)]'
            }`}
          >
        ...
        {isHost && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between text-[10px] text-gray-500 px-1">
              <span className="uppercase tracking-wider">Villager readiness</span>
              <span className={`font-bold font-mono ${ allReady ? 'text-moon-safe' : 'text-yellow-500' }`}>
                {readyNonHostCount} / {nonHostPlayers.length} players ready
              </span>
            </div>
            <div className="w-full h-1 bg-moon-iron/30 rounded-full overflow-hidden">
              <div
                className="h-full bg-moon-safe transition-all duration-500 rounded-full"
                style={{ width: nonHostPlayers.length > 0 ? `${(readyNonHostCount / nonHostPlayers.length) * 100}%` : '0%' }}
              />
            </div>
            ...
            {!canStart && (
              <div className="p-3 rounded-lg bg-red-950/30 border border-red-500/20 text-xs text-red-400 flex items-center gap-2">
                ...
                <span>
                  {numPlayers < minPlayers
                    ? `Minimum ${minPlayers} villagers required.`
                    : 'Waiting for all villagers to confirm ready.'}
                </span>
              </div>
            )}
            <button
