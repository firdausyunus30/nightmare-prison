  ...
  return (
    <div className="min-h-screen px-4 py-8 flex flex-col justify-between overflow-hidden relative">
      ...
      <div className="w-full max-w-md mx-auto text-center relative">
        ...
        <p className="mt-5 text-[10px] font-mono tracking-[0.35em] uppercase text-red-400">Final lockdown report</p>
        <h1 className="mt-2 text-4xl creepster-font tracking-wider text-white text-glow-red uppercase">The Last Bell</h1>
        <p className="mt-3 text-xs tracking-widest uppercase text-gray-500">The prison falls silent. Read the final report.</p>
      </div>
      ...
      >
        <div className="flex items-center gap-2 pb-3 mb-4 border-b border-red-500/20">
          ...
          <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-red-300">Incident report: classified</span>
        </div>
        <div className="flex gap-3">
          <Skull className="w-5 h-5 text-red-700 shrink-0 mt-0.5" />
          <p className="text-sm leading-7 text-gray-200 italic">{story || 'The iron gates groan shut. Somewhere in the darkness, the final truth waits to be uncovered.'}</p>
        </div>
        <p className="mt-5 text-[10px] leading-relaxed text-gray-500 uppercase tracking-wider">
          The identities are still sealed. One final verdict remains.
        </p>
      </motion.section>
      ...
      <div className="w-full max-w-md mx-auto relative">
        <button
          onClick={onReveal}
          className="w-full py-3.5 rounded-xl bg-prison-danger hover:bg-red-500 border border-red-400/30 text-white font-bold text-sm uppercase tracking-[0.15em] transition-all flex items-center justify-center gap-2 box-glow-red active:scale-95"
        >
