...
export default function LandingPage({ onCreateRoom, onJoinRoom, connectionUnavailable }) {
  ...
  const handleCreate = async (e) => {
    ...
    setLoading(true);
    try {
      const configuredRoles = Object.values(gameOptions.roles).reduce((total, count) => total + count, 0);
      if (gameOptions.minPlayers > gameOptions.maxPlayers) {
        setError('Minimum players cannot be higher than the player limit.');
        return;
      }
      if (configuredRoles > gameOptions.minPlayers) {
        setError('Configured character total must not exceed minimum players.');
        return;
      }
